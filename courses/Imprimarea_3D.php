...
<html>
	...
	<body class = "flex flex-col gap-5 sm:gap-6 pb-24">
		...
		<div class = "flex flex-col justify-center items-center gap-8 sm:gap-10">
            ...
            <p class = "text-sm sm:text-base md:text-lg md:text-lg text-center w-86 sm:w-105 md:w-150"><?php echo $course['description']?></p>
            <?php foreach($sections as $section): ?>
                ...
                <?php switch($section['sectionType']):
                    case "subtitle": ?>
                        <p class = "text-1.5xl md:text-2.5xl font-bold text-primaryColor"><?php echo $sectionData['0']['content']?></p>
                    <?php break ?>
                    <?php case "text": ?>
                        <p class = "text-sm sm:text-base md:text-lg w-86 sm:w-105 md:w-150"><?php echo nl2br($sectionData['0']['content'])?></p>
                    <?php break ?>
                    <?php case "image": ?>
                        <img src="<?php echo 'uploads/'.$sectionData[0]['content']?>" alt="" class = "w-86 sm:w-105 h-50 sm:h-60 md:w-150 md:h-84">
                    <?php break ?>
                    <?php case "textImage": ?>
                        <p class = "text-sm sm:text-base md:text-lg w-86 sm:w-105 md:w-150"><?php echo nl2br($sectionData[0]['content'])?></p>
                        <img src="<?php echo 'uploads/'.$sectionData[1]['content']?>" alt="" class = "w-86 sm:w-105 h-50 sm:h-60 md:w-150 md:h-84">
                    <?php break ?>
                    <?php case "tripleImageDescription": ?>
                        <?php for($i=0; $i<=5; $i++): ?>
                            <?php if($i%2 == 1): ?>
                                <img src="<?php echo 'uploads/'.$sectionData[$i]['content']?>" alt="" class = "w-86 sm:w-105 h-50 sm:h-60 md:w-150 md:h-84">
                            <?php else: ?>
                                <p class = "text-sm sm:text-base md:text-lg w-86 sm:w-105 md:w-150"><?php echo nl2br($sectionData[$i]['content'])?></p>
                            <?php endif ?>
                        <?php endfor ?>
                    <?php break ?>
                    ...
                    <?php break ?>
                    <?php case "video": ?>
                        <iframe class = "w-86 sm:w-105 h-50 sm:h-60 md:w-150 md:h-84" src="<?php 'uploads/'.$sectionData[0]['content']?>" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
                    <?php break ?>
                <?php endswitch ?>
            <?php endforeach ?>
		</div>
